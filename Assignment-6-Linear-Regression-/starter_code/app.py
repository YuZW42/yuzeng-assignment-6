...
def generate_plots(N, mu, sigma2, S):
    ...
    Y = true_relationship + error  # Replace with code to generate random values for Y with specified mean and variance

    # TODO 2: Fit a linear regression model to X and Y
    # Hint: Use Scikit Learn
    
    model = LinearRegression() # the function is imported from above 
    model.fit(X, Y)
    ...
    for _ in range(S):
        ...
        X_sim = X_sim.reshape(-1,1) # this is the same as above

        # TODO: Generate Y values with normal additive error (mean mu, variance sigma^2)
        true_relationship = 2 * X + 3 # try different constant 
        error = np.random.normal(mu, np.sqrt(sigma2), N).reshape(-1, 1)
        Y_sim = true_relationship + error  # Replace with code to generate Y values
